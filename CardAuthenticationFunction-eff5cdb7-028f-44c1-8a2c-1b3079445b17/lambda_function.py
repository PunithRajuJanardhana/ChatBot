...
def dispatch(intent_request):
    ...
    if intent['name'] == 'FallbackIntent':
        return dialog.elicit_slot('CardNumber', active_contexts, session_attributes, init_state, [{'contentType': 'PlainText', \
                                    'content': "I didn't follow that. Please say or enter your card number."}])
    
    if intent['name'] == 'MissingInfo':
        return dialog.elicit_slot('CardNumber', active_contexts, session_attributes, init_state, [{'contentType': 'PlainText', \
                                    'content': "For security, you must provide required information. \
                                    If you need time to get that information, say, wait a moment. \
                                    Otherwise please say or enter, your card number."}])
                                    
    if intent['name'] == 'Authentication':
